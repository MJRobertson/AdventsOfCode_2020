...
bool IsGreater(int target, int num1, int num2)
{
    return num1 + num2 > target;
}

bool DoesSum(int targetAmount, int num1, int num2)
{
...
int main()
{
    ...
    for (int i = 0; i < v.size() - 1; ++i)
    {
        ...
        {
            if (IsGreater(requiredAmount, v[i], v[j]))
            {
                break;
            }

            if (DoesSum(requiredAmount, v[i], v[j]))
            {
